...
public class PatrolWalk_3 : MonoBehaviour {

    public float movePower = 4f;//움직이는 속도
    float startPos;//시작위치
    float endPos = 13f;//끝위치
    float distance;
    Animator animator;
    Vector3 movement;
    Renderer rend;
    ...
    public void detected_3()//player가 경비원이랑 부딪히면 실행되는 메소드
    {

        if (user.move == true)//player가 숨어있지 않을때
        {
            Debug.Log("Game Over : Patrol 3");
        }
        else//player가 숨어있지 않을때
        {
            Debug.Log("Not Detected : Patrol 3");
        }
    }

    IEnumerator changeSpeed()
    {
        Debug.Log("SpeedUp : Patrol 3");
        //속도를 올린다
